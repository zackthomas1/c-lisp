...
lval* lenv_get(lenv* e, lval* k) {
    ...
    // if no symbol found check parent environment
    if (e->par) {
...
lval* lval_call(lenv* e, lval* f, lval* a) {
    // if builtin then call function directly
...
lval* builtin_eval(lenv* e, lval* a) {
    LASSERT_NUM("\\", a, 1);
    LASSERT(a , (a->cell[0]->type == LVAL_QEXPR || a->cell[0]->type == LVAL_SEXPR),
        "Function 'eval' passed incorrect type for argument."
        "Got %s expecting %s or %s",
        lval_type(a->cell[0]->type), lval_type(LVAL_QEXPR), lval_type(LVAL_SEXPR));
...
lval* builtin_lambda(lenv* e, lval* a) {
  /* Check Two arguments, each of which are Q-Expressions */
    ...
    for(int i = 0; i < a->cell[0]->cell[0]->count; i++){
        LASSERT(a, (a->cell[0]->cell[0]->cell[i]->type == LVAL_SYM), 
            "Can not define non-symbol. GOT %s, expected %s",
            lval_type(a->cell[0]->cell[0]->cell[i]->type), lval_type(LVAL_SYM));
    }

    lval* formals = lval_pop(lval_pop(a, 0), 0);
    lval* body = lval_pop(lval_pop(a, 0), 0);
